...
def normalize
     ...
     #R.quit
    # R = RinRuby.new(@executable="/home5/engeebe1/R")
...
EOF
  ...
  @resultsToView.pop
  logger.debug @resultsToView.to_s
 ...
 def get_paths(id)
     ...
     probe_file_name = upload.probe_file_name

     #set the path to the file folder
     calib_path = "#{Rails.root}/public/calibs"
     probe_path = "#{Rails.root}/public/probes"
      
     #create file paths and return them    
     calib_file = File.join(calib_path, calib_file_name)
     probe_file = File.join(probe_path, probe_file_name)
 
     return calib_file, probe_file
 end
